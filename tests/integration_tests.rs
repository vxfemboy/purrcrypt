use purrcrypt::cipher::efficient::{
    EfficientPetCipher, EmotionalContext, FileType, PetDialect as EfficientPetDialect,
};
...
fn test_complete_encryption_pipeline() {
    ...
    println!(
        "   ✓ Data compressed ({} bytes -> {} bytes, {:.2}x ratio)",
        test_data.len(),
        compressed_data.len(),
    ...
    rp_cipher
        .encode_data(&encrypted_message.to_bytes(), &mut rp_encoded)
        .unwrap();
    let rp_encoded_str = String::from_utf8(rp_encoded).unwrap();
    println!(
        "   ✓ Data encoded as steganographic RP ({} bytes -> {} bytes, {:.2}x ratio)",
        encrypted_message.to_bytes().len(),
        rp_encoded_str.len(),
    ...
    let decrypted_data = deserialized_message
        .decrypt(
...
fn test_emotional_context_integration() {
    ...
    for context in &contexts {
        ...
        let efficient_cipher = EfficientPetCipher::new_with_context(
            EfficientPetDialect::Kitty,
            FileType::Unknown,
            *context,
        );
        let mut encoded = Vec::new();
        efficient_cipher
            .encode_data(test_data, &mut encoded)
            .unwrap();
        let encoded_str = String::from_utf8(encoded).unwrap();
...
fn test_large_data_integration() {
    ...
    efficient_cipher
        .encode_data(&large_data, &mut efficient_encoded)
        .unwrap();
    let efficient_encoded_str = String::from_utf8(efficient_encoded).unwrap();

    let efficient_decoded = efficient_cipher
        .decode_data(&efficient_encoded_str)
        .unwrap();
...
fn test_unicode_integration() {
    ...
    let mut rp_encoded = Vec::new();
    rp_cipher
        .encode_data(unicode_data, &mut rp_encoded)
        .unwrap();
    let rp_encoded_str = String::from_utf8(rp_encoded).unwrap();
    ...
    let mut efficient_encoded = Vec::new();
    efficient_cipher
        .encode_data(unicode_data, &mut efficient_encoded)
        .unwrap();
    let efficient_encoded_str = String::from_utf8(efficient_encoded).unwrap();
...
fn test_performance_integration() {
    ...
    println!(
        ...
        rp_encode_time, rp_decode_time
    );
    println!(
        "     Expansion ratio: {:.2}x",
        rp_encoded_str.len() as f64 / test_data.len() as f64
    );
    ...
    let mut efficient_encoded = Vec::new();
    efficient_cipher
        .encode_data(&test_data, &mut efficient_encoded)
        .unwrap();
    let efficient_encoded_str = String::from_utf8(efficient_encoded).unwrap();
    ...
    println!(
        "     Encode time: {:?}, Decode time: {:?}",
        efficient_encode_time, efficient_decode_time
    );
    println!(
        "     Expansion ratio: {:.2}x",
        efficient_encoded_str.len() as f64 / test_data.len() as f64
    );
...
fn test_cli_integration() {
    ...
    let encrypted_message =
        SecureMessage::encrypt(test_data, &keypair.ecdh_public, &keypair.kyber_public).unwrap();

    let decrypted_data = encrypted_message
        .decrypt(&keypair.ecdh_secret, &keypair.kyber_secret)
        .unwrap();
