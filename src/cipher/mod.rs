...
impl AnimalCipher {
    ...
    pub fn process_data<W: Write>(
        ...
        while i < data.len() {
            ...
            if remaining >= 3 {
                ...
                if cfg!(test) {
                    println!(
                        "DEBUG ENCODE: Chunk [{}, {}, {}], packed value=0x{:x}",
                ...
                if cfg!(test) {
                    println!("DEBUG ENCODE: Chunk [{}], packed value=0x{:x}", byte, byte);
                }
            ...
            CipherMode::Decrypt => {
                ...
                while i < words.len() {
                    ...
                    if remaining >= 4 {
                        ...
                        if cfg!(test) {
                            println!(
                                "DEBUG DECODE: Word {}, pattern_index={}, six_bits={}",
    ...
    fn decode_word_with_fallback(
        ...
        if let Some(decoded_value) = decoded {
            ...
            let expected_pattern_index = expected_position
                % match self.current_dialect {
...
mod tests {
    ...
    fn test_basic_encryption_decryption() {
        ...
        cipher
            .process_data(test_data, &mut encrypted, CipherMode::Encrypt)
            .unwrap();
        let encrypted_str = String::from_utf8(encrypted.clone()).unwrap();

        // Decrypt
        let decrypted = cipher
            .process_string(&encrypted_str, CipherMode::Decrypt)
            .unwrap();
        println!(
            "Original: {:?}, Encrypted: {:?}, Decrypted: {:?}",
            test_data, encrypted, decrypted
        );
    ...
    fn test_pattern_consistency() {
        ...
        for value in 0..64u8 {
            let mut encrypted = Vec::new();
            cipher
                .process_data(&[value], &mut encrypted, CipherMode::Encrypt)
                .unwrap();
            let encrypted_str = String::from_utf8(encrypted.clone()).unwrap();
    ...
    fn test_multi_byte_encryption() {
        ...
        for test_case in test_cases {
            let mut encrypted = Vec::new();
            cipher
                .process_data(&test_case, &mut encrypted, CipherMode::Encrypt)
                .unwrap();
            let encrypted_str = String::from_utf8(encrypted.clone()).unwrap();
            ...
            println!(
                "Original: {:?}, Encrypted: {:?}, Decrypted: {:?}",
                test_case, &encrypted, &decrypted
            );
            assert_eq!(
                test_case.as_slice(),
                decrypted.as_slice(),
                "Failed for case {:?}: encrypted as '{}'",
                test_case,
                encrypted_str.trim()
            );
        }
    ...
    fn test_dialect_switching() {
        ...
        cat_cipher
            .process_data(test_data, &mut cat_encrypted, CipherMode::Encrypt)
            .unwrap();
        ...
        dog_cipher
            .process_data(test_data, &mut dog_encrypted, CipherMode::Encrypt)
            .unwrap();
        ...
        assert_ne!(
            cat_str, dog_str,
            "Cat and dog dialects produced identical output"
        );

        // But both should decrypt correctly
        let cat_decrypted = cat_cipher
            .process_string(&cat_str, CipherMode::Decrypt)
            .unwrap();
        let dog_decrypted = dog_cipher
            .process_string(&dog_str, CipherMode::Decrypt)
            .unwrap();

        println!(
            "Original: {:?}, Encrypted: {:?}, Decrypted: {:?}",
            test_data, &cat_encrypted, &cat_decrypted
        );
        println!(
            "Original: {:?}, Encrypted: {:?}, Decrypted: {:?}",
            test_data, &dog_encrypted, &dog_decrypted
        );
    ...
    fn test_edge_cases() {
        ...
        let mut encrypted = Vec::new();
        cipher
            .process_data(&[], &mut encrypted, CipherMode::Encrypt)
            .unwrap();
        ...
        let mut encrypted = Vec::new();
        cipher
            .process_data(&[42], &mut encrypted, CipherMode::Encrypt)
            .unwrap();
        let encrypted_str = String::from_utf8(encrypted.clone()).unwrap();
        let decrypted = cipher
            .process_string(&encrypted_str, CipherMode::Decrypt)
        ...
        let mut encrypted = Vec::new();
        cipher
            .process_data(&[0xAA, 0x55], &mut encrypted, CipherMode::Encrypt)
            .unwrap();
        let encrypted_str = String::from_utf8(encrypted.clone()).unwrap();
        let decrypted = cipher
            .process_string(&encrypted_str, CipherMode::Decrypt)
