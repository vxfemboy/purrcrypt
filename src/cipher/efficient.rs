...
impl EfficientPetCipher {
    ...
    pub fn new_with_context(dialect: PetDialect, file_type: FileType, mood: EmotionalContext) -> Self {
        let mut cipher = Self {
            dialect,
            file_type,
            mood,
    ...
    fn initialize_sound_mappings(&mut self) {
        match self.dialect {
            PetDialect::Kitty => {
                ...
                self.add_sound("meow meow", 0, SoundType::MessageStart, EmotionalContext::Greeting);
                self.add_sound("trill trill", 0, SoundType::MessageEnd, EmotionalContext::Content);
                self.add_sound("purr purr", 0, SoundType::WordBoundary, EmotionalContext::Playful);
                self.add_sound("hiss", 0, SoundType::ErrorMarker, EmotionalContext::Frustrated);
                ...
                self.add_sound("mrow", 5, SoundType::NeutralSound, EmotionalContext::Request);
                self.add_sound("nya", 6, SoundType::NeutralSound, EmotionalContext::Excited);
                self.add_sound("mew", 7, SoundType::NeutralSound, EmotionalContext::Attention);
                
                // Extended sounds (8-11) - energetic, playful
                self.add_sound("prrr", 8, SoundType::ExcitedSound, EmotionalContext::Excited);
                self.add_sound("chatter", 9, SoundType::ExcitedSound, EmotionalContext::Playful);
                self.add_sound("mrrrow", 10, SoundType::ExcitedSound, EmotionalContext::Excited);
                self.add_sound("brrr", 11, SoundType::ExcitedSound, EmotionalContext::Content);
                
                // Extended sounds (12-15) - satisfied, purring
                self.add_sound("meoww", 12, SoundType::ContentSound, EmotionalContext::Content);
                self.add_sound("mroww", 13, SoundType::ContentSound, EmotionalContext::Content);
                self.add_sound("nyaa", 14, SoundType::ContentSound, EmotionalContext::Calm);
                self.add_sound("meww", 15, SoundType::ContentSound, EmotionalContext::Playful);
                
                // Additional sound types for completeness
                self.add_sound("yowl", 0, SoundType::UpsetSound, EmotionalContext::Frustrated);
                self.add_sound("mrow?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                self.add_sound("nya?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                self.add_sound("mew?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                self.add_sound("hiss", 0, SoundType::AlertSound, EmotionalContext::Alert);
                self.add_sound("chirr", 0, SoundType::PlayfulSound, EmotionalContext::Playful);
            },
            PetDialect::Puppy => {
                // Structure markers - unique patterns that don't conflict with data
                self.add_sound("woof woof", 0, SoundType::MessageStart, EmotionalContext::Greeting);
                self.add_sound("howl howl", 0, SoundType::MessageEnd, EmotionalContext::Content);
                self.add_sound("ruff ruff", 0, SoundType::WordBoundary, EmotionalContext::Playful);
                self.add_sound("growl", 0, SoundType::ErrorMarker, EmotionalContext::Frustrated);
                
                // Basic nibble mapping (0-15) - direct sound to nibble mapping
                self.add_sound("tail-wag", 0, SoundType::HappySound, EmotionalContext::Content);
                ...
                self.add_sound("bark", 5, SoundType::NeutralSound, EmotionalContext::Request);
                self.add_sound("arf", 6, SoundType::NeutralSound, EmotionalContext::Attention);
                self.add_sound("wrf", 7, SoundType::NeutralSound, EmotionalContext::Playful);
                
                // Extended sounds (8-11) - energetic, playful
                self.add_sound("wooof", 8, SoundType::ExcitedSound, EmotionalContext::Excited);
                self.add_sound("baark", 9, SoundType::ExcitedSound, EmotionalContext::Playful);
                self.add_sound("arff", 10, SoundType::ExcitedSound, EmotionalContext::Excited);
                self.add_sound("wrff", 11, SoundType::ExcitedSound, EmotionalContext::Playful);
                
                // Extended sounds (12-15) - satisfied, relaxed
                self.add_sound("woooof", 12, SoundType::ContentSound, EmotionalContext::Content);
                self.add_sound("baaark", 13, SoundType::ContentSound, EmotionalContext::Content);
                self.add_sound("arfff", 14, SoundType::ContentSound, EmotionalContext::Calm);
                self.add_sound("wrfff", 15, SoundType::ContentSound, EmotionalContext::Content);
                
                // Question sounds (checksums) - curious, questioning
                self.add_sound("ruff?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                self.add_sound("woof?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                self.add_sound("arf?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                self.add_sound("yip?", 0, SoundType::QuestionSound, EmotionalContext::Curious);
                
                // Additional sound types for completeness
                self.add_sound("snarl", 0, SoundType::UpsetSound, EmotionalContext::Frustrated);
                self.add_sound("growl", 0, SoundType::AlertSound, EmotionalContext::Alert);
                self.add_sound("play-bow", 0, SoundType::PlayfulSound, EmotionalContext::Playful);
            }
        }
    }

    fn add_sound(&mut self, pattern: &str, bits: u8, sound_type: SoundType, _emotional_context: EmotionalContext) {
        let mapping = SoundMapping {
            pattern: pattern.to_string(),
            sound_type: sound_type.clone(),
        };
        self.sound_map.push(mapping);
        self.reverse_map.insert(pattern.to_string(), (bits, sound_type));
    }
        ...
        match self.dialect {
            PetDialect::Kitty => {
                ...
                self.filler_words = vec![
                    "little".to_string(), "big".to_string(), "happy".to_string(),
                    "soft".to_string(), "warm".to_string(), "cozy".to_string(),
                    "bright".to_string(), "shiny".to_string(), "smooth".to_string(),
                ];
            ...
            PetDialect::Puppy => {
                ...
                // Filler words for natural flow
                self.filler_words = vec![
                    "good".to_string(), "big".to_string(), "happy".to_string(),
                    "soft".to_string(), "warm".to_string(), "nice".to_string(),
                    "bright".to_string(), "fun".to_string(), "great".to_string(),
                ];
    ...
    fn write_contextual_greeting<W: Write>(&self, writer: &mut W) -> io::Result<()> {
        if !self.sentence_variations.is_empty() {
            ...
            let greeting = &self.sentence_variations[greeting_index % self.sentence_variations.len()];
    ...
    fn decode_nibble(&self, word: &str) -> io::Result<u8> {
        ...
        if cfg!(test) {
            println!("DEBUG: '{}' not found in reverse_map. Available keys: {:?}", word, self.reverse_map.keys().collect::<Vec<_>>());
        }
        ...
        for (index, &pattern) in sound_patterns.iter().enumerate() {
            if pattern == word {
                ...
                if cfg!(test) {
                    println!("DEBUG: Decoded basic '{}' to nibble value {}", word, nibble_value);
                }
                return Ok(nibble_value);
            }
        }
        
        Err(io::Error::new(io::ErrorKind::InvalidData, format!("Unknown sound: {}", word)))
    }
    ...
    fn is_contextual_word(&self, word: &str) -> bool {
        ...
        let contextual_words = [
            "Oh", "Look", "What", "So", "Let", "This", "I'll", "Time", "Here's", "I", "Wow!", "amazing!", "check", "out.", "seems", "important.",
            "be", "careful", "with", "this.", "like", "fun!", "have", "some", "fun!",
            "little", "big", "happy", "soft", "warm", "cozy", "bright", "shiny", "smooth",
            "good", "nice", "great"
        ];
...
mod tests {
    ...
    fn test_kitty_encoding() {
        ...
        cipher.encode_data(single_byte, &mut single_encoded).unwrap();
    ...
    fn test_efficiency() {
        ...
        println!("Expansion ratio: {:.2}x", encoded_str.len() as f64 / test_data.len() as f64);
    ...
    fn test_all_emotional_contexts() {
        ...
        for context in &contexts {
            let cipher = EfficientPetCipher::new_with_context(PetDialect::Kitty, FileType::Unknown, context.clone());
    ...
    fn test_all_file_types() {
        ...
        for file_type in &file_types {
            let cipher = EfficientPetCipher::new_with_context(PetDialect::Kitty, file_type.clone(), EmotionalContext::Calm);
    ...
    fn test_compression_ratio_efficient() {
        ...
        let test_cases = [
            ...
            ("Random data", b"\x01\x23\x45\x67\x89\xAB\xCD\xEF".repeat(10)),
            ("Mixed data", b"Hello World! 1234567890 !@#$%^&*()".repeat(20)),
        ];
        ...
        for (name, data) in &test_cases {
            ...
            println!("{}: {} bytes -> {} bytes ({}x expansion)", 
                    name, data.len(), encoded_str.len(), expansion_ratio);
            
            // Should be reasonable expansion (not 1000%+ like the old system)
            assert!(expansion_ratio < 20.0, "Expansion ratio too high for {}: {}x", name, expansion_ratio);
    ...
    fn test_sound_type_mapping() {
        ...
        for sound_type in &sound_types {
            let sounds = cipher.sound_map.iter()
                .filter(|m| std::mem::discriminant(&m.sound_type) == std::mem::discriminant(sound_type))
                .collect::<Vec<_>>();
            assert!(!sounds.is_empty(), "No sounds found for type: {:?}", sound_type);
        }
    }
    ...
    fn test_performance_benchmark_efficient() {
        let cipher = EfficientPetCipher::new(PetDialect::Kitty);
        let test_data = b"Performance test data for benchmarking the efficient cipher system.".repeat(100);
        ...
        println!("  Encode speed: {:.2} MB/s", test_data.len() as f64 / encode_time.as_secs_f64() / 1_000_000.0);
        println!("  Decode speed: {:.2} MB/s", test_data.len() as f64 / decode_time.as_secs_f64() / 1_000_000.0);
        
        assert_eq!(decoded, test_data);
        
        // Performance assertions
        assert!(encode_time.as_secs_f64() < 1.0, "Encoding too slow: {:?}", encode_time);
        assert!(decode_time.as_secs_f64() < 1.0, "Decoding too slow: {:?}", decode_time);
    }
