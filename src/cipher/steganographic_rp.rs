...
impl SteganographicRPCipher {
    ...
    fn initialize_action_metadata(&mut self) {
        ...
        self.action_metadata.insert("*purrs softly*".to_string(), 0x00); // File type: text
        self.action_metadata.insert("*purrs loudly*".to_string(), 0x01); // File type: binary
        ...
        self.action_metadata.insert("*tail swish*".to_string(), 0x04);   // Encryption: ChaCha20
    ...
    fn initialize_phrase_dictionary(&mut self) {
        ...
        self.phrase_dictionary.insert("*nuzzles*".to_string(), vec![0x00, 0x00, 0x00, 0x00]);
        self.phrase_dictionary.insert("*purrs*".to_string(), vec![0xFF, 0xFF, 0xFF, 0xFF]);
        self.phrase_dictionary.insert("*headbonk*".to_string(), vec![0xCA, 0xFE, 0xBA, 0xBE]);
        self.phrase_dictionary.insert("*kneads paws*".to_string(), vec![0x00, 0x00, 0x00, 0x00]);
        self.phrase_dictionary.insert("*sleepy yawn*".to_string(), vec![0xFF, 0xFF, 0xFF, 0xFF]);
    }
    ...
    fn initialize_asterisk_actions(&mut self) {
        ...
        self.asterisk_actions.insert("**excited**".to_string(), 0x04);  // Priority flag
        self.asterisk_actions.insert("***sleepy***".to_string(), 0x05); // Compression flag
    }
    ...
    fn initialize_repetition_patterns(&mut self) {
        ...
        self.repetition_patterns.insert("mew mew mew".to_string(), 0x02);       // Repeat last word
        self.repetition_patterns.insert("purr purr purr purr".to_string(), 0x03); // Run-length encoding
    }
    ...
    pub fn encode_data<W: Write>(&self, data: &[u8], writer: &mut W) -> io::Result<()> {
        ...
        let total_chunks = (data.len() + 3) / 4; // Calculate total chunks
        let mut chunk_index = 0;
        
        for chunk in data.chunks(4) { // Process 4 bytes at a time
    ...
    fn encode_chunk_as_phonemes(&self, chunk: &[u8]) -> String {
        ...
        for (i, &byte) in chunk.iter().enumerate() {
            ...
            let high_phonemes = self.nibble_to_phonemes_with_capitalization(high_nibble, byte & 0x01);
            let low_phonemes = self.nibble_to_phonemes_with_capitalization(low_nibble, (byte >> 1) & 0x01);
            ...
            let extra_bits = (byte & 0x03) as u8;
    ...
    fn nibble_to_phonemes(&self, nibble: u8) -> String {
        ...
        let phoneme_pairs = match self.dialect {
            PetDialect::Kitty => [
                "me", "ew", "ow", "rr", "pr", "tr", "ch", "ny",
                "ya", "wa", "ma", "na", "ra", "la", "ka", "ha"
            ],
            PetDialect::Puppy => [
                "wo", "of", "ar", "ru", "ba", "gr", "hu", "ya",
                "wa", "ma", "na", "ra", "la", "ka", "ha", "bo"
            ]
        };
    ...
    fn create_emotional_progression_template(&self, chunk_index: usize, total_chunks: usize) -> String {
    ...
    fn write_metadata_action<W: Write>(&self, writer: &mut W, chunk_index: usize) -> io::Result<()> {
    ...
    pub fn decode_data(&self, input: &str) -> io::Result<Vec<u8>> {
        ...
        let has_rp_markers = words.iter().any(|word| word.starts_with('*') && word.ends_with('*'));
    ...
    fn is_phoneme_word(&self, word: &str) -> bool {
        ...
        let blacklisted_words = match self.dialect {
            PetDialect::Kitty => [
                "hewwo", "hello", "hi", "hey", "oh", "ooh", "aww", "cute", "kitty",
                "cat", "feline", "whiskers", "paws", "tail", "ears", "fluffy", "soft",
                "the", "and", "but", "for", "are", "with", "this", "that", "from"
            ],
            PetDialect::Puppy => [
                "woof", "bark", "hewwo", "hello", "hi", "hey", "oh", "ooh", "aww", "cute", "puppy", 
                "dog", "canine", "tail", "paws", "ears", "fluffy", "soft",
    ...
    fn decode_phoneme_sequence_simple(&self, words: &[&str]) -> Option<Vec<u8>> {
            ...
            if clean_word.len() >= 2 {
                ...
                if clean_word.len() == 4 {
                    ...
                    if let Some(nibble1) = self.phonemes_to_nibble(first_phoneme) {
                        ...
                        if cfg!(test) {
                            println!("DEBUG: Could not decode phoneme: '{}' (clean: '{}', len: {})", word, clean_word, clean_word.len());
                        }
                    }
                } else {
                    // Debug: print what we're trying to match
        ...
        if cfg!(test) {
            println!("DEBUG: Decoded {} nibbles into {} bytes", nibbles.len(), bytes.len());
        }
...
mod tests {
    ...
    fn test_personality_differences() {
        ...
        chatty_kitty.encode_data(test_data, &mut chatty_encoded).unwrap();
        let chatty_str = String::from_utf8(chatty_encoded).unwrap();
        
        let mut excited_encoded = Vec::new();
        excited_kitty.encode_data(test_data, &mut excited_encoded).unwrap();
    ...
    fn test_advanced_steganography() {
        ...
        assert!(encoded_str.contains("!") || encoded_str.contains("?") || encoded_str.contains("~")); // Punctuation encoding
    ...
    fn test_emotional_progression() {
        ...
        assert!(encoded_str.contains("*tired") || encoded_str.contains("*sleepy")); // End tired
    }
    ...
    fn test_all_personalities() {
        ...
        for personality in &personalities {
            ...
            // Test decoding
            let decoded = cipher.decode_data(&encoded_str).unwrap();
            assert_eq!(decoded, test_data, "Failed for personality: {:?}", personality);
    ...
    fn test_all_dialects() {
        ...
        for dialect in &dialects {
            for personality in &personalities {
                ...
                // Test decoding
                let decoded = cipher.decode_data(&encoded_str).unwrap();
                assert_eq!(decoded, test_data, "Failed for dialect: {:?}, personality: {:?}", dialect, personality);
                
                println!("Dialect {:?}, Personality {:?}: {}", dialect, personality, encoded_str);
            }
        }
    }
    ...
    fn test_phoneme_encoding_accuracy() {
        ...
        for nibble in 0..=15u8 {
            ...
            assert_eq!(Some(nibble), decoded_nibble, "Failed for nibble: {}", nibble);
        }
        ...
        for nibble in 0..=15u8 {
            for cap_bit in 0..=1u8 {
                ...
                let decoded_nibble = cipher.phonemes_to_nibble(&phonemes);
                assert_eq!(Some(nibble), decoded_nibble, "Failed for nibble: {}, cap_bit: {}", nibble, cap_bit);
            }
        }
    }
    ...
    fn test_repetition_patterns() {
        ...
        assert!(cipher.repetition_patterns.contains_key("purr purr purr purr"));
    }
    ...
    fn test_natural_pause_generation() {
        ...
        assert!(pause_str.contains("purr") || pause_str.contains("chirp") || pause_str.contains("nuzzle"));
    }
    ...
    fn test_metadata_action_generation() {
        ...
        assert!(action_str.contains("nuzzle") || action_str.contains("purr") || action_str.contains("tail"));
    }
    ...
    fn test_greeting_and_ending() {
        ...
        assert!(ending_str.contains("curl") || ending_str.contains("sleepy") || ending_str.contains("yawn"));
    }
    ...
    fn test_performance_benchmark() {
        let cipher = SteganographicRPCipher::new(PetDialect::Kitty, PetPersonality::Chatty);
        let test_data = b"Performance test data for benchmarking the steganographic RP system.".repeat(100);
