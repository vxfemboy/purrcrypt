...
mod tests {
    ...
    fn test_compression_algorithms() {
        ...
        let test_cases = [
            ...
            (
                ...
                b"Test data with some repetition. ".repeat(50),
            ),
            (
                "Low compressibility",
                b"\x01\x23\x45\x67\x89\xab\xcd\xef".repeat(20),
            ),
            (
                "Random data",
    ...
    fn test_compression_detection() {
        ...
        for (i, data) in already_compressed_cases.iter().enumerate() {
            ...
            assert_eq!(
                data, &result,
                "Already compressed data {} should be returned unchanged",
                i
            );
        }

        // Test uncompressed data (use repetitive data that will actually compress)
        let uncompressed = b"This is uncompressed data that should be compressed. "
            .repeat(10)
            .into_iter()
            .collect::<Vec<u8>>();
        ...
        assert_ne!(
            uncompressed, result,
            "Uncompressed data should be compressed"
        );
    }
    ...
    fn test_compression_quality() {
        ...
        println!(
            "Repetitive data compression ratio: {:.2}x",
            compression_ratio
        );
        assert!(
    ...
    fn test_streaming_compression_advanced() {
        ...
        for (i, data) in test_cases.iter().enumerate() {
            ...
            let mut decompressed = Vec::new();
            compressor
                .decompress_stream(std::io::Cursor::new(&compressed), &mut decompressed)
                .unwrap();

            assert_eq!(data, &decompressed, "Failed for test case {}", i);

            println!(
                "Streaming test {}: {} bytes -> {} bytes",
                i,
                data.len(),
                compressed.len()
            );
    ...
    fn test_compression_performance() {
        ...
        println!(
            "  Compress speed: {:.2} MB/s",
            test_data.len() as f64 / compress_time.as_secs_f64() / 1_000_000.0
        );
        println!(
            "  Decompress speed: {:.2} MB/s",
            test_data.len() as f64 / decompress_time.as_secs_f64() / 1_000_000.0
        );
        ...
        assert!(
            compress_time.as_secs_f64() < 1.0,
            "Compression too slow: {:?}",
            compress_time
        );
        assert!(
            decompress_time.as_secs_f64() < 1.0,
    ...
    fn test_entropy_analysis() {
        ...
        let test_cases = [
            ...
            ("Two values", [0u8, 1u8].repeat(50), 1.0),
            ("Four values", [0u8, 1u8, 2u8, 3u8].repeat(25), 2.0),
            (
                "Random",
                (0..100).map(|_| rand::random::<u8>()).collect(),
                8.0,
            ),
        ];
        ...
        for (name, data, expected_entropy) in &test_cases {
            ...
            if *expected_entropy < 7.0 {
                assert!(
                    (entropy - expected_entropy).abs() < 0.5,
    ...
    fn test_compression_fallback() {
        ...
        let test_data =
            b"Test data that might not compress well with zstd but should work with zlib";
    ...
    fn test_streaming_edge_cases() {
        ...
        let mut compressed = Vec::new();
        compressor
            .compress_stream(std::io::Cursor::new(empty_data), &mut compressed)
            .unwrap();
        ...
        let mut compressed = Vec::new();
        compressor
            .compress_stream(std::io::Cursor::new(single_byte), &mut compressed)
            .unwrap();
